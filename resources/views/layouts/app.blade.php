...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // =========================================
    // 🔥 GEOJSON INDONESIA (OUTLINE TIPIS)
    // =========================================
        ...
        if (item.latitude && item.longitude) {
            ...
            if (!isNaN(lat) && !isNaN(lng)) {
                ...
                const customIcon = L.divIcon({
                    className: '',
                    html: `<div id="node-${item.id_site}" class="node-glow"></div>`
                });
    ...
    // =========================================
    // 🔥 GARIS NETWORK (SEMUA NODE TERHUBUNG)
    // =========================================
    for (let i = 0; i < markersCoordinates.length; i++) {
        for (let j = i + 1; j < markersCoordinates.length; j++) {

            // glow layer
            L.polyline([markersCoordinates[i], markersCoordinates[j]], {
                color: '#00eaff',
                weight: 6,
                opacity: 0.08
            }).addTo(mapHome);
...
function updateStatus() {
    fetch(`/api/check-status?ip=${ip}`)
        ...
        .then(data => {
            ...
            const node = document.getElementById(`node-${siteId}`);

            if (!node) return;

            // RESET CLASS
            node.classList.remove('node-online', 'node-offline', 'node-unreachable');

            if (data.status === 'online') {

                node.classList.add('node-online');
                textStatus.innerText = 'ONLINE';
                textStatus.style.color = '#00ff9c';

            } else if (data.status === 'offline') {

                node.classList.add('node-offline');
                textStatus.innerText = 'RTO (Offline)';
                textStatus.style.color = '#ffaa00';

            } else {

                node.classList.add('node-unreachable');
                textStatus.innerText = 'UNREACHABLE';
                textStatus.style.color = '#ff3b3b';
            }
        });
}        updateStatus();
    ...
    // =========================================
    // 🔥 CONTROL INTERACTION (PUNYA KAMU)
    // =========================================
